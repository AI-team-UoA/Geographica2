...
# Check if ${ResultsBaseDir}/RDF4JSUT/RealWorld_Points exists and create it if necessary
if [ ! -d "${ResultsBaseDir}/RDF4JSUT/RealWorld_Points" ]; then
    mkdir -p "${ResultsBaseDir}/RDF4JSUT/RealWorld_Points"
else # clear existing data
    rm -r ${ResultsBaseDir}/RDF4JSUT/RealWorld_Points/*
fi

# return all arguments except the experiment name
echo "-bd \"${RDF4JRepoBaseDir}\" -rp realworld_points -cr false -r 3 -t 3600 -m 60 -l \"${ResultsBaseDir}/RDF4JSUT/RealWorld_Points\" run"