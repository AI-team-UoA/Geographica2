...
# Check if ${ResultsBaseDir}/RDF4JSUT/Synthetic_Pois exists and create it if necessary
if [ ! -d "${ResultsBaseDir}/RDF4JSUT/Synthetic_Pois" ]; then
    mkdir -p "${ResultsBaseDir}/RDF4JSUT/Synthetic_Pois"
else # clear existing data
    rm -r ${ResultsBaseDir}/RDF4JSUT/Synthetic_Pois/*
fi

# return all arguments except the experiment name
echo "-bd \"${RDF4JRepoBaseDir}\" -rp synthetic_pois -cr false -r 3 -t 3600 -l \"${ResultsBaseDir}/RDF4JSUT/Synthetic_Pois\" -N 1024 run"