...
# Check if ${ExperimentResultDir}/RealWorld exists and create it if necessary
if [ ! -d "${ExperimentResultDir}/RealWorld" ]; then
    mkdir -p "${ExperimentResultDir}/RealWorld"
#else # clear existing data
 #   rm -r ${ExperimentResultDir}/RealWorld/*
fi

# return all arguments except the experiment name
echo "-bd \"${RDF4JRepoBaseDir}\" -rp realworld -cr false -dr 0 -r 3 -t 3600 -m 60 -l \"${ExperimentResultDir}/RealWorld\" run"
